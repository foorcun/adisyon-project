...
@Component({
  selector: 'app-order-page-content-in-progress',
  standalone: true, // If using standalone components
  imports: [CommonModule, FormsModule],
  templateUrl: './order-page-content-in-progress.component.html',
  styleUrls: ['./order-page-content-in-progress.component.scss']
})
export class OrderPageContentInProgressComponent implements OnInit {
  orders: Order[] = [];
  ...
  private subscribeToOrders(): void {
    this.orderPageFacadeService
      ...
      .pipe(
        ...
        tap((orders) => {
          this.orders = orders.filter((order) => order.status === OrderStatus.IN_PROGRESS);
        })
      )
      .subscribe({
